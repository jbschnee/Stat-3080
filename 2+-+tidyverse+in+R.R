...
mult_data <- data.frame(Factor=c("ByOne", "ByTwo", "ByThree", "ByFour", "ByFive"), 
                        Fives=c(5,10,15,20,25), 
...
filter(mult_data, Fives>15)
filter(mult_data, Fives>15, Threes>12)
filter(mult_data, Fives>15 | Threes>12)
filter(mult_data, Fives>15 | Fives<7)
...
summarize(mult_data, s.Fives=sum(Fives), s.Threes=sum(Threes))

#  The group_by() function is often used with the summarize() function  #
#  to obtain summaries for differing groups.                            #

Davis2 <- group_by(Davis, sex)
summarize(Davis2, avg.height=mean(height), avg.weight=mean(weight))
